...
namespace BifServiceExpenditureMaterials.Forms
{
    ...
    /// </summary>
    public partial class FormDol : Window
    {
        ...
        public FormDol(string НомерЯчейки, string Ячейка, bool isWrite, int year)
        {
        ...
        public FormDol(Material material, CountMaterials countmaterial, string nomer, string yache)
        {
            InitializeComponent();
        ...
        private async void AddData()
        {
...

            {
                ...
                string message = $"*{(material.НомерЯчейки.Split("_")[1].Length == 1 ? "0" + material.НомерЯчейки.Split("_")[1] : material.НомерЯчейки.Split("_")[1])}.{GetMonth(HomePage.CurrentMounth)}.{DateTime.Now.Year}* на машине {Pattern.GetWordAndInteger(material.НомерЯчейки.Split("_")[0])} произведена *Доливка*. Было использовано: \r\n" +
                   subvalue +
                   oilmess +
                   anti +
                   filters + "\r\n" +
